...
SELECT  d.fname fname, d.lname lname, COUNT(mid) numMovies
FROM movie_directors md NATURAL JOIN (SELECT id mid FROM movie WHERE year >= 1990 AND year <= 2010) m, directors d
WHERE d.id = did
GROUP BY did
